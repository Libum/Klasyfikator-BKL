...
#' lematach, tagach gramatycznych lub zawierających oba. UWAGA: bi-gramy i tf-idf nie mogą być stosowane razem!!!


create_sets = function(train, test, sparsity = 0.99, type = "both", all = TRUE, bigram = c(FALSE,FALSE), tfidf = FALSE, results = TRUE){
        library(tm)
        library(RWeka)
        ...
        control_lem = ifelse(tfidf == TRUE, parse(text = "list(weighting = function(x) weightTfIdf(x, normalize = FALSE))"), control_lem)
        control_tag = ifelse(tfidf == TRUE, parse(text = "list(weighting = function(x) weightTfIdf(x, normalize = FALSE))"), control_tag)
        ...
        if (type == "both"){
                ...
                if (results ==TRUE){test_lem$Wynik = NULL}
                ...
                if (results ==TRUE){.GlobalEnv$test_lem$Wynik = test_tag$Wynik}
                rm(list = c("DTM_train_tag", "DTM_test_tag", "Corpus_train_tag", "Corpus_test_tag"))
                rm(list = c("DTM_train_lem", "DTM_test_lem", "Corpus_train_lem", "Corpus_test_lem"))
                if (all == FALSE){
                        rm(list = c("train_lem", "test_lem", "train_tag", "test_tag"), envir = .GlobalEnv)
                }
        }
}